...
public class ProductServiceImpl implements ProductService {
    ...
    @Override
    public ResponseDto addProductToCart(Integer productId, Integer userId) {
        log.info("ProductServiceImpl.addProductToCart invoked with productId: {}, userId: {}", productId, userId);
        ResponseDto responseDto = null;
        try {
            if (productId == null || userId == null) {
                log.info("Invalid product id or user id provided.");
                return serviceUtil.getErrorServiceResponse(
                        ApplicationMessageConstants.ServiceErrorMessages.ERR_RETRIEVE_ALL_PRODUCT_DETAILS);
            }
            ...
            // Set the addToCart flag and userId
            productDto.setAddToCart(true);
            // Note: In a real implementation, you might want to verify the user owns this product
            // or create a separate cart table. For now, we'll use the existing product table.
            
            ProductDto savedProductDto = productServiceBL.updateProduct(productDto);
            if (savedProductDto != null) {
                log.info("Product added to cart successfully.");
                responseDto = serviceUtil.getServiceResponse("Product added to cart successfully");
            } else {
                log.info("Unable to add product to cart.");
                responseDto = serviceUtil.getErrorServiceResponse(
                        ApplicationMessageConstants.ServiceErrorMessages.ERR_RETRIEVE_ALL_PRODUCT_DETAILS);
            }
        } catch (Exception e) {
    ...
    @Override
    public ResponseDto removeProductFromCart(Integer productId, Integer userId) {
        log.info("ProductServiceImpl.removeProductFromCart invoked with productId: {}, userId: {}", productId, userId);
        ResponseDto responseDto = null;
        try {
            if (productId == null || userId == null) {
                log.info("Invalid product id or user id provided.");
            ...
            // Set the addToCart flag to false
            productDto.setAddToCart(false);
            
            ProductDto savedProductDto = productServiceBL.updateProduct(productDto);
            if (savedProductDto != null) {
                log.info("Product removed from cart successfully.");
                responseDto = serviceUtil.getServiceResponse("Product removed from cart successfully");
            } else {
                log.info("Unable to remove product from cart.");
                responseDto = serviceUtil.getErrorServiceResponse(
                        ApplicationMessageConstants.ServiceErrorMessages.ERR_RETRIEVE_ALL_PRODUCT_DETAILS);
            }
        } catch (Exception e) {
    ...
    @Override
    public ResponseDto getProductsInCartByUserId(Integer userId) {
        log.info("ProductServiceImpl.getProductsInCartByUserId invoked with userId: {}", userId);
        ResponseDto responseDto = null;
        try {
            if (userId == null) {
            ...
            List<ProductDto> productsInCart = productDao.findByUserIdAndAddToCartTrue(userId);
            if (productsInCart != null && !productsInCart.isEmpty()) {
                log.info("Retrieved {} products in cart for user {}.", productsInCart.size(), userId);
                responseDto = serviceUtil.getServiceResponse(productsInCart);
            } else {
                log.info("No products found in cart for user {}.", userId);
                responseDto = serviceUtil.getServiceResponse("No products in cart");
            }
        } catch (Exception e) {
            log.error("Exception occurs while retrieving products in cart.", e);
            responseDto = serviceUtil.getExceptionServiceResponseByProperties(
                    ApplicationMessageConstants.ServiceErrorMessages.EX_RETRIEVE_ALL_PRODUCT_DETAILS);
        }
        return responseDto;
